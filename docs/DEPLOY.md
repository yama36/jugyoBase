...
| 添付 PUT が CORS で失敗                              | `s3.identfill.com` の Nginx 設定で `Access-Control-Allow-Origin` が `https://identfill.com` になっているか・OPTIONS が 204 を返すか確認。ブラウザに `contains multiple values 'https://identfill.com, https://identfill.com'` と出る場合は **Nginx と MinIO の両方で CORS を付けている**。同梱の `s3.identfill.conf`（`proxy_hide_header` 入り）を `/etc/nginx/sites-available/` にコピーして `nginx -t` → `reload`。`grep proxy_hide_header /etc/nginx/sites-available/s3.identfill.conf` で反映を確認。コミュニティ版 MinIO では **`mc cors set` / `mc cors remove` は使えない**（有料 AIStor のみ）。OSS ではクラスタ全体の `MINIO_API_CORS_ALLOW_ORIGIN` と Nginx の `add_header` が重なることが多い → **どちらか一方にする**（推奨: Nginx のみ。MinIO に `MINIO_API_CORS_ALLOW_ORIGIN` があれば compose から外して `up -d minio`）。 |
