...
| お問い合わせで `yamalog.net` の CORS / 403           | ブラウザは **`/jugyobase/api/contact-form/*`**（同一オリジン）経由。`NEXT_PUBLIC_CONTACT_FORM_*` を yamalog 直 URL にしない。デプロイ後 `curl -sS https://identfill.com/jugyobase/api/contact-form/config` が 200 か確認。上流は `CONTACT_FORM_UPSTREAM_BASE`（未設定時 `https://yamalog.net/api/contactForm`）。 |
...
| 添付 PUT が **403**（CORS は通る）                    | Network のレスポンス XML の `<Code>` を確認（画面にも表示される）。**`x-amz-checksum-crc32` 付き URL** → `.env.production` の `AWS_REQUEST_CHECKSUM_CALCULATION=WHEN_REQUIRED` と compose の `app.environment` を確認し `build app --no-cache` → `up -d`。**`SignatureDoesNotMatch`** → `S3_ENDPOINT` と `MINIO_SERVER_URL` がどちらも `https://s3.identfill.com` か、Nginx が `Host` を書き換えていないか。**`AccessDenied`** → `jugyobase-app` のシークレット一致・バケット `jugyobase` への Put 権限（`mc pipe` で確認）。PUT は **Content-Type ヘッダなし**の `arrayBuffer`（`src/lib/s3.ts` / `AttachmentUploader.tsx`）。古いイメージが Content-Type 付き PUT のままなら再デプロイ。 |
