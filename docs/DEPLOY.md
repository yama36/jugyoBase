...
| 添付 PUT が **403**（CORS は通る）                    | Network のレスポンス XML の `<Code>` を確認。**`x-amz-checksum-crc32` 付き** → `AWS_REQUEST_CHECKSUM_CALCULATION=WHEN_REQUIRED` で app 再起動。**checksum 無しで 403** → 多くは `SignatureDoesNotMatch`（`S3_ENDPOINT` と `MINIO_SERVER_URL` が `https://s3.identfill.com` か）か Content-Type 未署名（`src/lib/s3.ts` の `signableHeaders` 入りで `build app` → `up -d`）。`mc pipe` で `S3_ACCESS_KEY_ID` の書き込み可否も確認。 |
