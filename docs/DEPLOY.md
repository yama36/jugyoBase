...
| 添付 PUT が **403**（CORS は通る）                    | 署名 URL に `x-amz-checksum-crc32` が付いているか確認。AWS SDK v3 が自動付与するがブラウザは送らない。**`src/lib/s3.ts` の `requestChecksumCalculation: "WHEN_REQUIRED"`** 入りイメージで `docker compose build app` → `up -d`。新しい presign URL に checksum クエリが無いこと。 |
