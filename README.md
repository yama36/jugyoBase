...
学校単位のクローズドな「授業・AI活用・業務改善」共有（MVP）。Next.js・PostgreSQL・Google OAuth・S3 互換ストレージ。
...
- 投稿の編集・削除は **作成者**、または **管理者（`admin`）**。管理者向けに統計・ユーザー管理・校設定・指導要領単元マスタ・**AI/ICT活用授業アンケートの CSV エクスポート**（`/t/{slug}/admin/export`）などの画面がある。
